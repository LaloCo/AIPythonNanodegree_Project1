...
def print_results(results_dic, results_stats_dic, model, 
    ...
    for stat in results_stats_dic:
        if stat[0] == "p":
            stat_name = ""
            for word in stat.split('_')[1:]:
                stat_name = stat_name + word + " "
            print("% {}: {}".format(stat_name.strip(), results_stats_dic[stat]))
        
    if print_incorrect_dogs and (results_stats_dic["n_correct_dogs"] + results_stats_dic["n_correct_notdogs"] != results_stats_dic["n_images"]):
        print("\n\nIncorrectly classified as dogs:")
        for dog in results_dic:
            if sum(results_dic[dog][3:]) == 1:
                print("{} (classified as {})".format(results_dic[dog][0], results_dic[dog][1]))
          
    if print_incorrect_breed and (results_stats_dic["n_correct_dogs"] != results_stats_dic["n_correct_breed"]):
        print("\n\nIncorrectly classified breeds:")
        for dog in results_dic:
            if sum(results_dic[dog][3:]) == 2 and results_dic[dog][2] == 0:
                print("{} (classified as {})".format(results_dic[dog][0], results_dic[dog][1]))
